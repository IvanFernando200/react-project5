...
const PokedexPage = () => {
  ...
  const audioOpening = useRef();
  const handleStop = (e) => {
    if (audioOpening.current.muted) {
      e.target.textContent = "🔊";
      audioOpening.current.muted = false;
      audioOpening.current.paused = false;
    } else {
      e.target.textContent = "🔇";
      audioOpening.current.muted = true;
      audioOpening.current.paused = true;
    }
  };
  ...
  return (
    <div className="page">
      ...
      <audio
        ref={audioOpening}
        src="/pokemon-indigo-opening.mp3"
        autoPlay
        loop
        muted
      ></audio>
      <button onClick={handleStop} className="audio-opening">
        🔇
      </button>
      <ListPokePage
